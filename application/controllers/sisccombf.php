...
class Sisccombf extends CI_Controller {
	...
	public function index()
	{
            ...
            redirect(base_url('/sisccombf/eess/'), 'refresh');
	}
...
//echo $sql;die();
        ...
        public function eess(){
            ...
            $db=$this->load->database('sisccombf',TRUE);

            $sql = "SELECT 
	id_historico_consumo,
	estacion_servicio,
	combustible,
	fecha_consumo, 
	now() AS fecha_actual,
	fecha_consulta,
	CAST(extract(epoch from now() - fecha_consulta)/60 AS INT) AS minutos,
	placa, volumen_consumo
FROM 
	st_consumo_eess
WHERE 
	DATE(fecha_consumo) = DATE(NOW())
ORDER BY 
	fecha_consumo DESC
";
