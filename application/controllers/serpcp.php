...
class Serpcp extends CI_Controller {
...
");
        ...
        public function eventos_1()
	{
            ...
            $db=$this->load->database('serpcp',TRUE);
$sql="SELECT descripcion_rfn, nombre_desviacion, SUM(1) AS cantidad
FROM bt_serpcp
GROUP BY descripcion_rfn, nombre_desviacion
ORDER BY descripcion_rfn, nombre_desviacion
            ";
//echo $sql;die();
            $query = $db->query($sql);
            //echo $query;
            ...
            foreach ($query->result_array() as $row)
            {
            $org_filial = $row['descripcion_rfn'];
            $org_region = $row['nombre_desviacion'];
            $casos = $row['cantidad'];
	    ...
	    $db->close();
            ...
            $this->view_data['view_name'] = "view_eventos_1";
            $this->view_data['menu'] = "evento1";
            $this->load->view('output', $this->view_data);

	}
        public function eventos_3()
	{
            ...
            $db=$this->load->database('serpcp',TRUE);
$sql="SELECT nombre_desviacion, descripcion_tipo, SUM(1) AS cantidad
FROM bt_serpcp
GROUP BY nombre_desviacion, descripcion_tipo
ORDER BY nombre_desviacion, descripcion_tipo
...
//echo $sql;die();
            ...
            foreach ($query->result_array() as $row)
            {
            $org_filial = $row['nombre_desviacion'];
            $org_region = $row['descripcion_tipo'];
            $casos = $row['cantidad'];
	    ...
	    $db->close();
            ...
            $this->view_data['view_name'] = "view_eventos_3";
            $this->view_data['menu'] = "evento3";
            $this->load->view('output', $this->view_data);
